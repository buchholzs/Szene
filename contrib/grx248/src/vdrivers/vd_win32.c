...
static HBITMAP CreateDIB8(HDC hdc, int w, int h, char **pBits)
{
    ...
    hBmp = CreateDIBSection(hdc, pbmInfo, DIB_RGB_COLORS, (void*)pBits, 0, 0);
    free(pbmInfo);
    return(hBmp);
}
...
static int init(char *options)
{
	...
	colorTableMutex = CreateMutex(NULL,  FALSE, NULL);
