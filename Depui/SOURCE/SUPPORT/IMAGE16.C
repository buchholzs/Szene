...
void Mx_ImagePixel16Fast(MxImage * image, int x1, int y1, MxColor color)
{
	 ...
	 ((unsigned short *) image->rows[y1])[x1] = color;
}
...
MxColor Mx_ImageGetPixel16Fast(MxImage * image, int x1, int y1)
{
	 ...
	 return ((unsigned short *) image->rows[y1])[x1];
}
...
static void _empty_aquire(struct MxImage *image)
{
	 ...
	 image->aquired = MxTrue, image->wasaquired = MxTrue;
}
...
{
	 return Mx_ImageCreateSimple(w, h, sizeof(unsigned short) * 8, &MxVtable16);
}
