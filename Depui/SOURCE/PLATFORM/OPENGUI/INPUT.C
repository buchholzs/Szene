...
int mx_opengui_kbd_init(void)
{
	 ...
	 if (regs.x.ax) {
		  ...
		  /* Center the mouse */
		  const int w = mx_desk->position.x2 - mx_desk->position.x1;
		  const int h = mx_desk->position.y2 - mx_desk->position.y1;
...
void mx_opengui_mouse(int *mx, int *my, int *mb)
{
	 if (mouse_valid) {
		  int x, y;
		  __dpmi_regs regs;
		  ...
		  *mb = regs.x.bx;
		  ...
		  if ((x != 0) || (y != 0)) {
				...
				*mx = MID(0, *mx, mx_desk->position.x2);
				*my = MID(0, *my, mx_desk->position.y2);
				mouse_mx = COORD_TO_MICKEY_X(*mx);
				mouse_my = COORD_TO_MICKEY_Y(*my);

				/* Move the mouse on the screen */
				Mx_MouseMove(*mx, *my, mx_opengui_screen_pixel, mx_opengui_screen_get_pixel);
		  }
	 }
}
