...
#include "depui/platform/support/mouse.h"
...
void mx_tinyptc_mouse(int *mx, int *my, int *mb)
{
	 if (mouse_valid) {
		  int x, y;
		  __dpmi_regs regs;
		  ...
		  *mb = regs.x.bx;
		  ...
		  if ((x != 0) || (y != 0)) {
				...
				*my = MID(0, *my, MxDesk->position.y2);
				...
				Mx_MouseMove(*mx, *my, mx_tinyptc_screen_pixel, mx_tinyptc_screen_get_pixel);
		  }
	 }
}
