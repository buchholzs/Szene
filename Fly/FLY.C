...
void main() { 
  ...
  while (!done) { // main loop
    ...
    if (__djgpp_nearptr_enable() == 0) {
      dosmemput(framebuffer, 64000, 0xA0000);
    } else {
      memcpy((void *) __djgpp_conventional_base+0xA0000,framebuffer,64000);
    }
...
void set_palette(char *pal) {
  __asm__ __volatile__(" \
    ...
    jnz 0b"
  ::"S" (pal):"%eax", "%edx", "%ecx");
//   const int PALETTE_INDEX = 0x03c8; */
//   const int PALETTE_DATA = 0x03c9; */
//   int i; */
...
void set_mode13() {
  ...
  __asm__ __volatile__("int $0x10"::"a" (0x13):"%ebx", "%ecx", "%edx");
}

void set_mode3() {
  __asm__ __volatile__("int $0x10"::"a" (3):"%ebx", "%ecx", "%edx");
  __djgpp_nearptr_disable();
}

void mouse_init() {
  __asm__ __volatile__("subl %%eax, %%eax \n\t\
    int $0x33"
    :"=a" (mouse_avail), "=b" (mouse_buttons)::"%ecx", "%edx");
  if (!mouse_avail) {
